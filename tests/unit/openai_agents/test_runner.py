...
def _reset_tracer():
    """Reset the module-level _tracer so register_tracing() runs fresh each test."""
...
class TestTracing:
    async def test_register_tracing_installs_processor(self):
        """register_tracing() creates a TraceManager and sets OAI trace processors."""
        with patch("agents.set_trace_processors") as mock_set:
            oai_mod._tracer = None
            oai_mod.register_tracing()

            assert oai_mod._tracer is not None
            mock_set.assert_called_once()
            processors = mock_set.call_args[0][0]
            assert len(processors) == 1
            assert isinstance(processors[0], oai_mod.MotusTracingProcessor)

    async def test_register_tracing_is_idempotent(self):
        """Calling register_tracing() twice doesn't install a second processor."""
        with patch("agents.set_trace_processors") as mock_set:
            oai_mod.register_tracing()
            oai_mod.register_tracing()
    ...
    async def test_runner_calls_register_tracing(self, agent, mock_oai_runner):
        """Runner.run() triggers register_tracing()."""
        with patch.object(
            oai_mod, "register_tracing", wraps=oai_mod.register_tracing
        ) as spy:
    ...
    async def test_get_tracer_returns_trace_manager(self):
        ...
        assert tracer is oai_mod._tracer
