...
def _reset_tracer():
    ...
    saved = anthropic_mod._tracer
    anthropic_mod._tracer = None
    yield
    anthropic_mod._tracer = saved
...
def _make_in_process_submit(mock_parse_factory):
    ...
    async def _in_process_submit(self, import_path, message, state, *, timeout=0):
...
class TestAnthropicSDKServe:
    ...
    async def test_mcp_runner_message_round_trip(self):
        ...
        async def _mcp_submit(self, import_path, message, state, *, timeout=0):
