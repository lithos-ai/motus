...
|---------|-------------|-----|
| `LITHOSAI_API_URL not set` | Missing env var | `export LITHOSAI_API_URL=https://...` |
| `LITHOSAI_API_KEY not set` | Missing env var | `export LITHOSAI_API_KEY=lithos_...` |
| DNS resolution error | Wrong API URL | Verify URL in your account settings |
| 401/403 | Invalid or expired API key | Get new key from LITHOSAI console |
...
| `Could not import module: motus.claude_agent requires the Claude Agent SDK` | `claude-agent-sdk` not installed in container | Add `claude-agent-sdk` to `requirements.txt` and redeploy |
