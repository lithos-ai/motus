...
## Pattern 11: Serving and deploying

All framework agents can be served directly — point at the instance, no wrapper needed:
...
client = OpenAIChatClient()
my_agent = ReActAgent(client, "gpt-4o", tools=[greet])
```
...
# Cloud (first deploy — creates project, saves project_id to motus.toml)
motus login
motus deploy --name my-app agent:my_agent

# Cloud (subsequent deploys — reads from motus.toml)
motus deploy
```

Same for `ToolRunner`, Google ADK `Agent`, and OpenAI `Agent` instances.
...
```python
# agent.py — deploy with: motus deploy --name prod-support agent:agent
from motus.agent import ReActAgent
from motus.models import OpenAIChatClient
...
# Agent — deploy target: agent:agent (run_turn inherited from AgentBase)
