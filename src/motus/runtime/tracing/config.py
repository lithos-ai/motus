...
def _get_collection_level_default() -> CollectionLevel:
    ...
    # MOTUS_TRACING=1 → user debugging mode (detailed + export)
    if os.getenv("MOTUS_TRACING", "").lower() in ("1", "true", "yes"):
...
def _get_project_default() -> str | None:
    """Resolve project identifier: MOTUS_PROJECT env var, then motus.toml."""
...
def _get_build_default() -> str | None:
    """Resolve build identifier: MOTUS_BUILD env var, then motus.toml."""
...
class TraceConfig:
    ...
    1. User Debugging (via env vars):
        ...
        $ MOTUS_TRACING=1 MOTUS_TRACING_ONLINE=1 python script.py
