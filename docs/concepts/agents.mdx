...
On providers that support it (Anthropic today), the agent caches its prompt prefix automatically. Control this with `cache_policy`.

| Policy | What gets cached |
|---|---|
| `"none"` | Nothing |
| `"static"` | System prompt and tool definitions, 5 minute TTL |
| `"auto"` (default) | Static plus the conversation-turn prefix, 5 minute TTL |
| `"auto_1h"` | Same as auto but with a 1 hour TTL, best for long-running agents |
...
If any of your tools are backed by an MCP server or another lazy resource, use the agent as an async context manager. This opens and closes the tool connections cleanly.
...
For agents with only plain function tools, this is optional. For MCP-backed tools it is the recommended pattern, since the session must be connected before first use and torn down on exit.

## Constructor reference

<Accordion title="Full parameter list">
...
| `name` | `str \| None` | auto-inferred | Agent name, used in tracing and tool registration. If unset, Motus infers it from the caller's variable name or falls back to the class name. |
...
| `max_steps` | `int` | `20` | Max reasoning-and-acting cycles before the agent raises `RuntimeError` |
| `timeout` | `float \| None` | `None` | Wall-clock deadline in seconds; raises `TimeoutError` when exceeded |
...
| `input_guardrails` | `list[Callable]` | `[]` | Hooks run on the user prompt before the agent starts |
| `output_guardrails` | `list[Callable]` | `[]` | Hooks run on the final result |
...
| `step_callback` | `Callable \| None` | `None` | Async callback fired after each LLM step that has tool calls |
