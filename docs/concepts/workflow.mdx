...
```python
from motus.runtime import agent_task, resolve, shutdown
...
<Note>
  The blocking public API is `resolve()` and `AgentFuture.af_result()`. There is no `.result()` method. Inside an `@agent_task`, prefer `await`: calling `resolve()` from the runtime's own event loop raises `RuntimeError` to protect you from a deadlock.
</Note>
...
init()         # optional: starts the runtime thread and event loop
# ... run tasks ...
shutdown()     # cancels in-flight tasks and stops the event loop
```

If you skip `init()`, the runtime auto-initializes on the first `@agent_task` call. `shutdown()` stops the runtime: it signals the event loop to exit, cancels any in-flight tasks, and poisons their futures with `RuntimeError("Motus runtime is shutting down")`. Tasks do not drain; if you need them to finish, `resolve()` or `await` them first. `is_initialized()` tells you whether the runtime is currently running.
