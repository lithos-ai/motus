...
### `InputSchema` (recommended)

Define an `InputSchema` subclass with Pydantic `Field` descriptors. This gives you validation constraints, nested objects, enums, and rich descriptions all in one place.
...
The schema describes what the model sees; the function signature is what Motus calls when the tool runs. Make sure the Pydantic field names match the function parameter names so Motus can route the arguments correctly. Use `InputSchema` instead of plain `BaseModel` so Motus emits a cleaner JSON Schema.
