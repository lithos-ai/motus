...
The schema describes what the model sees; the function signature is what Motus calls when the tool runs. Make sure the field names match the parameter names so Motus can route the arguments correctly.

Motus also exports `InputSchema`, a thin `BaseModel` subclass that strips redundant fields (`title`, `$defs`, `anyOf` wrappers) from the generated JSON Schema to save tokens. Use whichever you prefer; both work identically.
