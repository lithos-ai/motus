---
title: "Serving"
description: "Expose any agent as an HTTP server with session-based conversations and process-per-request isolation."
---

Expose any agent as an HTTP server with session-based conversations using `motus serve`. Each message spawns a fresh worker subprocess, so your agent runs in complete isolation — no shared state between requests, and a crash in one turn never affects another.
...
<Tip>
For detailed CLI usage and all available flags, see the [CLI reference for `motus serve`](/reference/cli-serve).
</Tip>
