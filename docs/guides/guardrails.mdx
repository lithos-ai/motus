---
title: "Guardrails"
description: "Validate, transform, or block agent inputs and outputs with plain Python functions."
---

A guardrail is just a Python function you hand to an agent or a tool. There is no base class, no registration DSL. Motus calls it with the relevant value and interprets what the function returns.
...
Sync and async functions both work. Sync ones run on a background thread via `asyncio.to_thread`.
...
Input guardrails receive the user's prompt as a string. If the function also declares an `agent` parameter, Motus passes in the agent instance so the guardrail can read configuration or call helpers. Return a string to rewrite the prompt; raise `InputGuardrailTripped` to block the run.
...
Tool input guardrails run before a tool function executes. They receive keyword arguments whose names match the tool's own parameters.
...
To **modify** an argument instead of blocking, return a `dict`. Motus merges it into the tool's kwargs; keys you omit stay unchanged.
...
<Note>
  When a tool guardrail raises, Motus catches the exception and returns the message to the model as a `{"error": ...}` tool result. The agent keeps running and the model can adjust, so you do not need to wrap tool calls in `try`/`except`.
</Note>
